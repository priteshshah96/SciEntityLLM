...
generated_output_dir = './llama2_generated_entity_test'
full_text_output_dir = './llama2_full_generated_text_test'
error_folder = './error'  # Path to the error folder
...
class TextDataset(Dataset):
    def __init__(self, directory, files_to_process):
        self.directory = directory
        self.files = [f for f in files_to_process if os.path.exists(os.path.join(directory, f))]
...
for filename, text in tqdm(dataloader, desc="Processing files"):
    ...
    try:
        ...
        # Remove the processed file from the error folder
        error_file = os.path.join(error_folder, f"{filename[:-4]}_generated.json")
        if os.path.exists(error_file):
            os.remove(error_file)
            print(f"Removed {error_file} from error folder")
