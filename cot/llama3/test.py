...
5. Use the following format for each entity:
   * Entity Name
     Category: [Category]
     Reasoning: [Reasoning]
6. Don't forget to generate entities.

Here are the fewshot examples demonstrating this process (DO NOT use these entities in your response unless they appear in the new text):
...
def extract_entities(generated_text):
    ...
    # Split the text at the last occurrence of "Scientific Entities Identified:"
    parts = generated_text.split("Scientific Entities Identified:")
    if len(parts) < 2:
        print("Warning: 'Scientific Entities Identified:' not found in the generated text.")
        return entities
    
    # Process only the last part (i.e., after the last "Scientific Entities Identified:")
    ...
    for line in relevant_text.split('\n'):
        ...
        if line.startswith('*'):
            if current_entity:
                entity_name = current_entity.get('name', '').lower()
